...
const BusinessCardActions: React.FC<BusinessCardActionsProps> = ({
  ...
  <div className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 z-20 flex-col items-center gap-2 transition-all duration-300 ease-out translate-x-12 opacity-0 md:group-hover:translate-x-0 md:group-hover:opacity-100">
    {isBusinessAccount ? (
      ...
      <button
        className="w-12 h-10 bg-navbar-bg rounded-[12px] flex items-center justify-center transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-sage/30 border border-white/40 shadow-md hover:bg-navbar-bg/90 hover:scale-110"
      ...
      <button
        className={`w-12 h-10 bg-navbar-bg rounded-[12px] flex items-center justify-center transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-sage/30 border border-white/40 shadow-md ${hasReviewed ? 'opacity-50 cursor-not-allowed' : 'hover:bg-navbar-bg/90 hover:scale-110'}`}
