-- Align get_similar_businesses with new taxonomy columns (primary_subcategory_slug / primary_category_slug).
-- Drops legacy dependency on category / sub_interest_id columns so results are no longer empty.
