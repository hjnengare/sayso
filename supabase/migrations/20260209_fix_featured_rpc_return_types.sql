-- Fix get_featured_businesses RPC: explicit casts so return structure matches RETURNS TABLE.
-- Error "structure of query does not match function result type" usually from numeric precision.
...
'Returns only bucket winners (top 2 per category/bucket). Explicit casts fix structure mismatch.';
