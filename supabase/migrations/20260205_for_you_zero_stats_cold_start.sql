-- For You zero-stats: hard filters + description completeness.
-- 1. Hard filters: exclude missing category or subcategory; keep status = active and price_ranges.
-- 2. Preference scoring: subcategory +5, category +2, verified +1, badge +1, has image +0.5, has description +0.5.
-- 3. Diversity: top per category, interleave. Freshness: seed (e.g. daily) for stable rotation.
...
as $$
  with filtered as (
    ...
    where b.status = 'active'
      -- Hard filter: must have category and subcategory (no missing taxonomy)
      and b.primary_category_slug is not null
      and b.primary_category_slug <> ''
      and b.primary_subcategory_slug is not null
      and b.primary_subcategory_slug <> ''
  ...
  with_score as (
    select
      ...
      (
        ...
        + case when f.description is not null and trim(f.description) <> '' then 0.5 else 0 end
...
'For You zero-stats: preference + quality only. Hard filters: active, has category/subcategory, price_ranges. Score: subcategory +5, category +2, verified +1, badge +1, image +0.5, description +0.5. Diversity: top per category, interleave.';
