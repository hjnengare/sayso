-- Add Foreign Key Relationship from review_helpful_votes to profiles
-- This enables PostgREST to understand the relationship for joins
-- Note: review_helpful_votes.user_id currently references auth.users(id),
...
BEGIN
  ...
  ELSE
    RAISE EXCEPTION 'Failed to create foreign key constraint review_helpful_votes_user_id_fkey';
  END IF;
END $$;
