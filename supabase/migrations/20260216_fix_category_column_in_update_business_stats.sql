-- Fix update_business_stats function to use correct column name
-- The function was referencing 'category' which doesn't exist - should be 'primary_subcategory_slug'
...
BEGIN
  -- Get business category for category-specific comparisons (FIXED: use primary_subcategory_slug)
  ...
  IF business_category IS NOT NULL AND total_reviews > 0 THEN
    -- Get category average rating and business count (FIXED: use primary_subcategory_slug)
    SELECT 
  ...
  IF business_category IS NOT NULL AND category_business_count > 1 THEN
    ...
    -- Punctuality percentile: rank this business's punctuality_score vs category (FIXED: use primary_subcategory_slug)
    ...
    -- Friendliness percentile (FIXED: use primary_subcategory_slug)
    WITH category_tag_scores AS (
      SELECT 
        b.id as business_id,
        CASE 
    ...
    -- Trustworthiness percentile (FIXED: use primary_subcategory_slug)
    WITH category_tag_scores AS (
      SELECT 
        b.id as business_id,
        CASE 
    ...
    -- Cost Effectiveness percentile (FIXED: use primary_subcategory_slug)
    WITH category_tag_scores AS (
      SELECT 
        b.id as business_id,
        CASE 
